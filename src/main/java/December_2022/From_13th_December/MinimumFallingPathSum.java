package December_2022.From_13th_December;

public class MinimumFallingPathSum {
    public int minFallingPathSum(int[][] matrix) {
        // works for 38.49 test cases
        // fails at 38th test-case
        int numOfRows=matrix.length;
        int numOfColumns=matrix[0].length;
        if(numOfColumns==1 && numOfRows==1)return matrix[0][0];
        else if(numOfRows==1){
            int min=matrix[0][0];
            for(int i:matrix[0]){
                if(i<min)min=i;
            ...
            for(int i=0;i<numOfColumns;i++){
                row=0;
                int present=matrix[row][i];
                tempSum+=present;
                row++;
                int got=pathFinder(i,matrix,tempSum,row,sum,numOfRows,numOfColumns);//i=column,
                if(got<sum)sum=got;
                tempSum-=present;
    ...
    public int pathFinder(int column, int[][] matrix, int sum, int row,int prevSum,int totalRows,int totalColumns){
        int tempSum=sum;
        int pSum=prevSum;
        //params: 0,matrix,7,2,15,3,3
        if(row==totalRows-1){
            if(column==0){
                for(int i=0;i<2;i++){
                    int present=matrix[row][i];
            ...
            //in the first iteration:params, tempSum=2, column=0,row=1,prevSum=15,
            //in the second iteration:params, column=1, tempSum=1,row=1,prevSum=15
            if(column==0){
                for(int i=0;i<2;i++){
                    int present=matrix[row][i];
                    tempSum+=present;
                    int got=pathFinder(i,matrix,tempSum,row+1,pSum,totalRows,totalColumns);
                    if(got!=pSum)pSum=got;
                    tempSum-=present;
                }
            }else if(column>0 && column<totalColumns-1){
                for(int i=column-1;i<column+2;i++){
